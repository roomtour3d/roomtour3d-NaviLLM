...
class RoomTourDataset(LLaVADataset):
    name = "roomtour"

    def _load_data(self, config: Dict, data_dir: str):
        if config.RoomTour.DIR.startswith("/"):
            path = os.path.join(config.RoomTour.DIR, config.RoomTour.SPLIT[self.split])
        else:
            path = os.path.join(data_dir, config.RoomTour.DIR, config.RoomTour.SPLIT[self.split])
    ...
    def __getitem__(self, index:int) -> Dict[str, Any]:
        ...
        for d in sampled_images:
            fts = self.feat_db.get_image_feature(item["videoId"], d["image_id"])
            features.append(fts)
